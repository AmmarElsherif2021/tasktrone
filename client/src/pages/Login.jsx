/**
 * Login.jsx  (updated)
 * ──────────────────────────────────────────────────────────────
 * Rendered inside <AuthLayout> which provides:
 *   - min-h-screen centering (both axes)
 *   - max-w-md constraint
 *   - page background
 *
 * REMOVED:
 *   ✕  outer `min-h-screen flex items-center justify-center bg-gray-50 py-12 px-4`
 *      → AuthLayout owns this
 *   ✕  `max-w-md w-full space-y-8` wrapper
 *      → AuthLayout owns max-w-md; this component owns space-y-8
 *
 * Loading / session states return a self-contained card that
 * AuthLayout centers automatically.
 * ──────────────────────────────────────────────────────────────
 */
import { useState, useEffect, useRef } from 'react'
import { useNavigate, useLocation } from 'react-router-dom'
import { Auth } from '@supabase/auth-ui-react'
import { ThemeSupa } from '@supabase/auth-ui-shared'
import {Spinner} from "../Ui/Spinner"
import supabase from '../lib/supabaseClient'
import { signInWithGoogle, checkUserProfileStatus } from '../API/users'
import logo from '../assets/logo.svg'

const DEMO_MODE = import.meta.env.VITE_DEMO_MODE === 'true'



export function Login() {
  const [session,     setSession]     = useState(null)
  const [loading,     setLoading]     = useState(false)
  const [authLoading, setAuthLoading] = useState(false)
  const navigate  = useNavigate()
  const location  = useLocation()

  useEffect(() => {
    let mounted = true

    supabase.auth.getSession().then(({ data: { session } }) => {
      if (!mounted) return
      setSession(session)
      setAuthLoading(false)
      if (session) handlePostAuth(session.user)
    })

    const { data: { subscription } } = supabase.auth.onAuthStateChange(
      async (event, session) => {
        if (!mounted) return
        setSession(session)
        setAuthLoading(false)
        if (event === 'SIGNED_IN' && session?.user) await handlePostAuth(session.user)
      }
    )

    return () => { mounted = false; subscription.unsubscribe() }
  }, [])

  const handlingAuth = useRef(false)
  const handlePostAuth = async (user) => {
    if (handlingAuth.current) return
    handlingAuth.current = true
    try {
      const { needsCompletion } = await checkUserProfileStatus(user.id)
      if (needsCompletion) {
        navigate('/complete-profile', { state: { from: location.state?.from || '/' }, replace: true })
      } else {
        navigate(location.state?.from?.pathname || '/', { replace: true })
      }
    } catch {
      navigate('/complete-profile', { state: { from: location.state?.from || '/' }, replace: true })
    } finally {
      handlingAuth.current = false
    }
  }

  const handleGoogleSignIn = async () => {
    if (loading) return
    try {
      setLoading(true)
      await signInWithGoogle(`${window.location.origin}/login`)
    } catch (error) {
      alert('Failed to sign in with Google: ' + error.message)
      setLoading(false)
    }
  }

  // AuthLayout centers these cards automatically
  if (authLoading) {
    return <Spinner title="Loading…" subtitle="Checking authentication status." />
  }
  if (session) {
    return <Spinner title="Setting up your account…" subtitle="Please wait while we prepare your workspace." />
  }

  // Main login card — AuthLayout provides max-w-md centering
  return (
    <div className="w-full space-y-8">
      <div>
        <img src={logo} alt="Tasktrone" className="mx-auto mb-4 w-16" />
        <h2 className="text-center text-3xl font-extrabold text-gray-900">
          Sign in to Tasktrone
        </h2>
        <p className="mt-2 text-center text-sm text-gray-600">
          Manufacturing Kanban Tool
        </p>
      </div>

      <div className="auth-container bg-white p-8 rounded-lg shadow-md">
        {DEMO_MODE && (
          <div className="mb-4 p-3 rounded" style={{ background: '#f0fdf4', border: '1px solid #86efac' }}>
            <p className="text-sm font-semibold text-green-800 mb-1">🎯 Demo Access</p>
            <p className="text-xs text-green-700">Email: <code>demo@tasktrone.com</code></p>
            <p className="text-xs text-green-700">Password: <code>Demo1234!</code></p>
          </div>
        )}

        <button
          onClick={handleGoogleSignIn}
          disabled={loading}
          className="w-full flex items-center justify-center px-4 py-2 border border-gray-300 rounded-md shadow-sm bg-white text-sm font-medium text-gray-700 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
        >
          <svg className="w-5 h-5 mr-2" viewBox="0 0 24 24">
            <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
            <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
            <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
            <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
          </svg>
          {loading ? 'Signing in…' : 'Continue with Google'}
        </button>

        <div className="relative my-4">
          <div className="absolute inset-0 flex items-center">
            <div className="w-full border-t border-gray-300" />
          </div>
          <div className="relative flex justify-center text-sm">
            <span className="px-2 bg-white text-gray-500">Or continue with email</span>
          </div>
        </div>

        <Auth
          supabaseClient={supabase}
          appearance={{
            theme: ThemeSupa,
            variables: { default: { colors: { brand: '#404040', brandAccent: '#52525b' } } },
          }}
          providers={[]}
          redirectTo={window.location.origin}
        />
      </div>
    </div>
  )
}