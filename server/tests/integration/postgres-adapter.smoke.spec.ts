...
=======
  ...
  it("reads back seeded board/task/user rows with camelCase-mapped columns", async () => {
    const board = await seedBoard(adapter, { name: "Seeded Board" });
    const task = await seedTask(adapter, board.id, { title: "Seeded Task" });
    const user = await seedUser(adapter, {
      organizationId: board.organizationId,
    });

    const [foundTask] = await adapter.query<Task>(
    ...
    const foundBoard = await adapter.queryOne<Board>(
      "SELECT * FROM boards WHERE id = $1",
      [board.id],
    );
    expect(foundBoard?.name).toBe("Seeded Board");

    const foundUser = await adapter.queryOne<User>(
...
>>>>>>> e1a6194 (new file:   .github/ISSUE_TEMPLATE/task.yml)
    ...
    await adapter.query(
      "UPDATE adapter_smoke_test SET label = $1 WHERE id = $2",
      ["updated", inserted!.id],
    );
  ...
  it("rolls back the transaction when the callback throws", async () => {
    await expect(
      adapter.transaction(async (tx) => {
        await tx.query("INSERT INTO adapter_smoke_test (label) VALUES ($1)", [
          "should-rollback",
        ]);
  ...
  it("commits the transaction when the callback succeeds", async () => {
    ...
    const rows = await adapter.query<{ label: string }>(
      "SELECT * FROM adapter_smoke_test",
    );
