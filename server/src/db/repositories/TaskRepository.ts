...
export class TaskRepository {
  ...
  async findByBoard(boardId: string): Promise<Task[]> {
    return this.db.query<Task>('SELECT * FROM tasks WHERE board_id = $1 ORDER BY created_at DESC', [boardId])
  }
  ...
  async update(id: string, changes: UpdateTaskInput): Promise<Task | null> {
    ...
    if (changes.position3d !== undefined) {
      ...
      values.push(changes.position3d ? JSON.stringify(changes.position3d) : null)
    }
    ...
    return this.db.queryOne<Task>(`UPDATE tasks SET ${fields.join(', ')} WHERE id = $${i} RETURNING *`, values)
  }
}
